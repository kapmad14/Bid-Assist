...
def db_prefetch_existing_maps(
    bids: List[Dict[str, Any]]
) -> Tuple[Dict[int, Dict[str, Any]], Dict[str, Dict[str, Any]]]:
    """
    Bulk fetch existing tenders rows and return lookup maps:
    - by gem_bid_id
    - by bid_number
    """
    gem_ids = set()
    ...
    for b in bids:
        ...
        if not bn:
            continue
        bid_numbers.add(bn)
        gid = extract_gem_bid_id_from_bid_number(bn)
        if gid:
            gem_ids.add(gid)

    by_gem: Dict[int, Dict[str, Any]] = {}
    by_bid: Dict[str, Dict[str, Any]] = {}

    if not gem_ids and not bid_numbers:
        return by_gem, by_bid

    base_url = f"{SUPABASE_URL.rstrip('/')}/rest/v1/{TENDERS_TABLE}"
    CHUNK = 200

    # ---- Fetch by gem_bid_id ----
    gem_list = list(gem_ids)
    for i in range(0, len(gem_list), CHUNK):
        chunk = gem_list[i:i+CHUNK]
        params = {
            "gem_bid_id": f"in.({','.join(map(str, chunk))})",
            "select": "*"
        }
        resp = _db_request_with_retries("GET", base_url, AUTH_HEADERS, params=params)
        if resp.status_code == 200:
            for r in resp.json():
                if r.get("gem_bid_id"):
                    by_gem[r["gem_bid_id"]] = r
                if r.get("bid_number"):
                    by_bid[r["bid_number"]] = r

    # ---- Fetch by bid_number (covers rows missing gem_bid_id) ----
    bid_list = list(bid_numbers)
    for i in range(0, len(bid_list), CHUNK):
        chunk = bid_list[i:i+CHUNK]
        params = {
            "bid_number": f"in.({','.join(chunk)})",
            "select": "*"
        }
        resp = _db_request_with_retries("GET", base_url, AUTH_HEADERS, params=params)
        if resp.status_code == 200:
    ...
    root_logger.info(
        "Prefetch complete: %d rows by gem_id, %d rows by bid_number",
        len(by_gem), len(by_bid)
    )
...
def process_bids(
    ...
    max_candidates: int,
    existing_by_gem: Dict[int, Dict[str, Any]],
    ...
    for bid in bids:
        ...
        existing = None
        if gem_bid_id:
            existing = existing_by_gem.get(gem_bid_id)
        if not existing:
            existing = existing_by_bid.get(bid_number)
...
def main():
    ...
    root_logger.info("Prefetching existing DB rows...")
    existing_by_gem, existing_by_bid = db_prefetch_existing_maps(bids)
