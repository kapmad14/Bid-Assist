...
const InfoTooltip = ({ text }: { text: string }) => (
  <div className="relative group inline-block ml-1 cursor-pointer">
    ...
    <div
      className="absolute left-1/2 -translate-x-1/2 mt-1 hidden group-hover:block whitespace-nowrap z-20
...
function TendersContent() {
  ...
  const logUserEvent = useCallback(async (eventType: string, eventValue?: any) => {
    try {
  ...
  const handleSetActiveTab = (val: TabOption) => {
    setActiveTab(val);
    setCurrentPage(1);
  };

  // Helpers — operate on date-only YYYY-MM-DD strings (or fallback timestamp)
  const parseEndOfDay = (dateOrTs?: string | null): Date | null => {
  ...
  const isClosingSoon = (dateString?: string | null) => {
    const endOfDay = parseEndOfDay(dateString);
  ...
  const getTenderStatus = (dateString?: string | null): 'Closed' | 'Closing Soon' | 'Active' => {
    ...
    const endOfDay = parseEndOfDay(dateString);
    if (!endOfDay) return 'Active';
    const now = new Date();
    if (endOfDay.getTime() < now.getTime()) return 'Closed';
  ...
  const fetchTenders = useCallback(async () => {
    ...
    try {
      ...
      const { data, total } = await tenderStore.getTenders({
        ...
        statusFilter,
        sortBy,
        // emdFilter intentionally omitted per new mapping
  ...
  }, [currentPage, searchTerm, sortBy, activeTab, initialSource, recommendedOnly, logUserEvent]);
  ...
  const toggleShortlist = async (e: React.MouseEvent, tender: Tender) => {
    ...
    setTenders((current) => current.map((t) => (t.id === tender.id ? { ...t, isShortlisted: !t.isShortlisted } : t)));
    ...
    <div className="flex flex-col lg:flex-row gap-8 pb-12">
      ...
      <div className="w-full lg:w-72 flex-shrink-0 space-y-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4" aria-hidden={isLoading}>
          ...
          {/* Keyword Search */}
          <div className="mb-4">
            <label htmlFor="tender-search" className="text-xs font-bold text-gray-700 uppercase mb-1 block">
              Keyword Search
            </label>
          ...
          <div className="divide-y divide-gray-100">
            {/* EMD removed per requested mapping; keep other placeholder filters */}
            <FilterSection title="Other filters" isOpen>
              <div className="space-y-2">
                <p className="text-xs text-gray-500">Use the search box to filter by item, bid number, ministry or department.</p>
              </div>
            </FilterSection>
          </div>
          ...
          <div className="mt-4 bg-gray-50 border border-gray-100 rounded-lg p-3 text-xs leading-relaxed text-gray-600">
            ...
            <ul className="list-disc pl-4 mt-1 space-y-1">
              ...
              <li>
                <span className="font-semibold">Recommended for Me</span> 🔶 — Shows tenders based on items you’ve added in your{' '}
                <span className="underline font-medium">Catalogue</span>.
              </li>
            </ul>

            {/* Optional subtle CTA */}
            <div className="mt-2">
              <Link href="/catalog" className="text-[11px] font-semibold text-[#0E121A] underline hover:text-black">
      ...
      <div className="flex-1 min-w-0" aria-busy={isLoading}>
              ...
              {['Active', 'Closing Soon', 'Shortlisted', 'All', 'Archived'].map((tab) => (
                <button
                  ...
                  className={`px-3 py-1.5 rounded-md text-sm font-medium transition-all ${
                    activeTab === tab ? 'bg-gray-200 text-[#0E121A] shadow-sm' : 'text-gray-500 hover:text-gray-700'
                  }`}
          ...
          <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
            ...
            <div className="text-sm text-gray-500 font-medium whitespace-nowrap hidden lg:block">
              {totalRecords === 0 ? (
                ...
                <>
                  Showing <span className="font-bold text-gray-900">{showingStart}</span> - <span className="font-bold text-gray-900">{showingEnd}</span> of{' '}
                  <span className="font-bold text-gray-900">{totalRecords}</span> tenders
                </>
              )}
            </div>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-4">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5">
              <button
                ...
                className={`flex items-center justify-center px-5 py-3 rounded-2xl font-medium transition focus:outline-none focus:ring-2 focus:ring-offset-2 ${
                  recommendedOnly ? 'bg-black text-white shadow' : 'bg-[#F7C846] text-[#0E121A] shadow-md hover:brightness-95'
                }`}
              >
            ...
            {showLoginPrompt && (
              <div role="alert" className="mt-2 text-xs text-gray-700 bg-yellow-50 border border-yellow-100 p-2 rounded">
                <div className="flex items-center gap-2">
                  ...
                  <Link href="/login" className="ml-2 text-sm font-semibold text-[#0E121A] underline">
                    Sign in
                  </Link>
                </div>
              </div>
            ...
            {recommendedOnly && <div className="text-xs text-gray-500 ml-2">Showing only tenders recommended for you</div>}
          </div>
        ...
        {isLoading ? (
          ...
          <div className="bg-white rounded-xl border border-dashed border-gray-300 p-12 text-center">
            ...
            <p className="text-gray-500">
              {recommendedOnly
                ...
                : 'Check your database connection or filters.'}
            </p>
            {debugError && (
          ...
          <div className="space-y-4">
            {tenders.map((tender) => {
              const urgent = isClosingSoon((tender as any).end_date as string | undefined);
              const timeLeft = getTimeLeft((tender as any).end_date as string | undefined);

              // compute status for the status pill (Closed / Closing Soon / Active)
              const computeStatus = (() => {
                const now = new Date();
                const deadline = (tender as any).end_date ? new Date((tender as any).end_date as string) : null;
                ...
                <div key={tender.id} className="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow p-5 group relative">
                  <div className="flex flex-col md:flex-row justify-between gap-4 mb-4">
                    ...
                    <div className="flex-1 pr-8">
                      <Link
                        ...
                        onClick={() =>
                          void logUserEvent('TENDER_VIEWED', {
                            tenderId: tender.id,
                            source: 'title'
                          })
                        }
                      >
                        <h3 className="text-lg font-bold text-[#0E121A] group-hover:text-blue-700 transition-colors line-clamp-2 mb-2 cursor-pointer">
                          {(tender as any).item || 'Untitled tender'}
                        </h3>
                      </Link>
                      ...
                      <div className="flex items-center gap-3 mb-2">
                        <span className="bg-gray-100 text-gray-700 text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wide border border-gray-200">
                          {(tender as any).bid_number || 'ID N/A'}
                        </span>
                        ...
                        <span className="flex items-center gap-1 bg-gray-50 text-gray-600 text-[10px] font-bold px-2 py-1 rounded border border-gray-200">
                          <FileText className="w-3 h-3" aria-hidden /> {(tender as any).quantity ? `${(tender as any).quantity} Qty` : 'Qty N/A'}
                        </span>
                    ...
                    {/* Right Info (PDF & Shortlist) */}
                    <div className="flex flex-col items-end justify-between shrink-0 gap-3 min-w-[140px]">
                      <div className="text-right">
                        <p className="text-xs font-bold text-gray-500 uppercase mb-0.5">PDF</p>
                        <p className="text-base font-semibold text-[#0E121A]">{(tender as any).pdf_uploaded ? 'Available' : 'Not uploaded'}</p>
                      </div>
                      ...
                      >
                        <Star className={`w-3 h-3 ${tender.isShortlisted ? 'fill-current text-yellow-500' : ''}`} aria-hidden />
                        {tender.isShortlisted ? 'Shortlisted' : 'Shortlist'}
                      </button>
                    </div>
                  </div>

                  {/* Footer Info — department in title, country removed */}
                  <div className="border-t border-gray-100 pt-4 grid grid-cols-1 md:grid-cols-2 gap-y-3 gap-x-6">
                    ...
                    <div className="flex items-start gap-2.5">
                      ...
                      <div>
                        <p className="text-sm font-bold text-gray-900 line-clamp-1">{(tender as any).department || 'Unknown'}</p>
                        {/* country / 'India' line removed */}
                      </div>
                    </div>
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2 text-xs text-gray-500">
                          ...
                          <span className="font-medium text-gray-700">
                            {(tender as any).published_date ? new Date((tender as any).published_date as string).toLocaleDateString() : 'N/A'}
                          </span>
                        </div>
                        <div className="flex items-center gap-2 text-xs text-gray-500">
                          ...
                          <span className="font-bold text-gray-900">
                            {(tender as any).end_date ? new Date((tender as any).end_date as string).toLocaleDateString() : 'N/A'}
                          </span>
                        </div>
                      </div>
